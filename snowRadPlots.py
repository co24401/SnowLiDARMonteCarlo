# -*- coding: utf-8 -*-
"""
Created on Fri Dec  4 21:53:32 2020
...
plt.ylim(bottom=0)
plt.legend(('Counts: $r_*$ = {}um'.format(1E6*snow_rads[0]), \
            'Counts: $r_*$ = {}um'.format(1E6*snow_rads[1]), \
            'Counts: $r_*$ = {}um'.format(1E6*snow_rads[2]), \
            'Diffusion: $r_*$ = {}um'.format(1E6*snow_rads[0]), \
            'Diffusion: $r_*$ = {}um'.format(1E6*snow_rads[1]), \
            'Diffusion: $r_*$ = {}um'.format(1E6*snow_rads[2])), loc='upper right')
plt.xlabel('Photon travel-time (ns)')
plt.ylabel('Counts')
plt.title(r'Time-of-flght histograms, $\rho_*$ = {} kg/m3, $\lambda$ = {} nm, Various $r_*$'.format(density_snow, wavelength*1E9))
plt.show()
...
plt.plot(1E9*bins[1:], 10*diff_curves_true[2, 1:]*np.log10(np.e) + \
         ...
         linestyle='--', color='tab:green')
plt.ylim(bottom=0)
plt.legend(('Fit: $r_*$ = {}um'.format(1E6*snow_rads[0]), \
            'Fit: $r_*$ = {}um'.format(1E6*snow_rads[1]), \
            'Fit: $r_*$ = {}um'.format(1E6*snow_rads[2]), \
            'True: $r_*$ = {}um'.format(1E6*snow_rads[0]), \
            'True: $r_*$ = {}um'.format(1E6*snow_rads[1]), \
            'True: $r_*$ = {}um'.format(1E6*snow_rads[2])), loc='upper right')
plt.xlabel('Photon travel-time (ns)')
plt.ylabel('Counts')
plt.title(r'Fitted vs. True Diffusion Curves, $\rho_*$ = {} kg/m3, $\lambda$ = {} nm, Various $r_*$'.format(density_snow, wavelength*1E9))
plt.show()
...
plt.axvline(density_snow, linestyle='--', color='y')
plt.xlabel('Snow density (kg/m3)')
plt.ylabel('log(loss)')
plt.title('Density Estimation Loss Function')
plt.legend((r'$r_*$ = {} um'.format(1E6*snow_rads[0]), \
            r'$r_*$ = {} um'.format(1E6*snow_rads[1]), \
            r'$r_*$ = {} um'.format(1E6*snow_rads[2]), \
            r'True $\rho_*$'))
...
for ii, r_snow in enumerate(snow_rads):
    ...
    with open(folder + 'counters_' + filename, 'rb') as f:
        ...
        plt.hist(data[ii, :], bins=500, log='True'); plt.title('$r_*$ = {}'.format(r_snow))
        plt.show()
...
# Time-of-flight histogram + Diffusion curves
plt.plot(1E9*bins, 10*np.log10(hists[0, :]*counters[0, 1]/counters[0, 0])); 
plt.plot(1E9*bins, 10*np.log10(hists[1, :]*counters[1, 1]/counters[1, 0]));
plt.plot(1E9*bins, 10*np.log10(hists[2, :]*counters[2, 1]/counters[2, 0]));
plt.plot(1E9*bins[1:], 10*diff_curves_true[0, 1:]*np.log10(np.e) + \
